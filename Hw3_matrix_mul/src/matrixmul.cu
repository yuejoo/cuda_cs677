...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	dim3 dimGrid((N.width-1)/BLOCK_WIDTH+1,(M.height-1)/BLOCK_WIDTH+1,1);
	dim3 dimBlock(BLOCK_WIDTH,BLOCK_WIDTH,1);
	// Launch the device computation threads!
	MatrixMulKernel<<<dimGrid,dimBlock>>>(Md,Nd,Pd,M.height,N.width,M.width);
...
void WriteFile(Matrix M, char* file_name)
{
	nocutWriteFilef(file_name, M.elements, M.width*M.height, 0.0001f, true);
}
